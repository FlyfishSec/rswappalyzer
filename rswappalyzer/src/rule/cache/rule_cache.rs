...
use crate::{RswError, RuleConfig, RuleSource, RuleStage};
use std::fs;
...
impl RuleCacheManager {
    ...
    pub fn load_from_cache(config: &RuleConfig) -> RswResult<RuleLibrary> {
        // 1. 内置规则/已编译规则 不支持从缓存加载
        if matches!(config.origin.source, RuleSource::Embedded)
            || matches!(config.origin.stage, RuleStage::Compiled)
        {
            return Err(RswError::InvalidInput(
                if config.origin.source == RuleSource::Embedded {
                    "Embedded rules do not support loading from cache."
                } else {
                    "Precompiled rules do not support loading from cache."
                }
                .to_string(),
            ));
        }
        ...
        // 3. 读取文件
        let cache_data = fs::read(&cache_file).map_err(|e| RswError::IoError(e))?;

        let cached_rules: Vec<CachedTechRule> =
            serde_json::from_slice(&cache_data).map_err(|e| RswError::JsonError(e.into()))?;

        Self::convert_cached_rules(cached_rules)
    }

    // 同步保存缓存（适配新 RuleOrigin 模型 + 已编译规则不保存缓存）
    pub fn save_to_cache(config: &RuleConfig, rule_lib: &RuleLibrary) -> RswResult<()> {
            ...
            || matches!(config.origin.stage, RuleStage::Compiled)
        {
            return Err(RswError::InvalidInput(
                if config.origin.source == RuleSource::Embedded {
                    "Saving embedded rules to cache is not supported."
                } else {
                    "Saving precompiled rules to cache is not supported."
                }
                .to_string(),
            ));
        }
        ...
        if let Some(parent_dir) = cache_file.parent() {
            fs::create_dir_all(parent_dir).map_err(|e| RswError::IoError(e))?;
        }

        // 4. 写入文件（此时 &cache_file 类型正确）
        let cache_data = Self::build_cached_rules(rule_lib)?;
        fs::write(&cache_file, cache_data).map_err(|e| RswError::IoError(e))?;
    ...
    fn convert_cached_rules(cached_rules: Vec<CachedTechRule>) -> RswResult<RuleLibrary> {
        ...
        for cached in cached_rules {
            let tech_name = cached.basic.tech_name.clone().ok_or_else(|| {
                RswError::InvalidInput(
                    "The cached rule is missing the required 'tech_name' field.".to_string(),
                )
            })?;
            ...
            for (scope, cached_scope_rule) in cached.rules {
                match_rules.insert(
                    scope.clone(),
                    MatchRuleSet::from_cached(&scope, cached_scope_rule),
                );
            }
        ...
        Ok(RuleLibrary {
            core_tech_map,
            category_rules: FxHashMap::default(),
        })
    }
    ...
    fn build_cached_rules(rule_lib: &RuleLibrary) -> RswResult<Vec<u8>> {
        ...
        for (_, parsed) in &rule_lib.core_tech_map {
            ...
            cached_rules.push(CachedTechRule {
                basic: parsed.basic.clone(),
                rules,
            });
        }
