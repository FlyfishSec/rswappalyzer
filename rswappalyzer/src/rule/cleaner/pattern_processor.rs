...
use crate::{
    RswResult, RswappalyzerError,
    rule::{
        core::{MatchType, ParsedTechRule, Pattern, PatternList, PatternMap},
        indexer::scope::MatchScope,
    },
};
use log::warn;
...
impl PatternProcessor {
    ...
    pub fn process_tech_rule_patterns(
        ...
        let js =
            self.build_keyed_pattern(original_tech, MatchScope::Js, stats, "js")?;

        Ok((url, html, scripts, script_src, meta, headers, cookies, js))
    }
    ...
    pub fn clean_and_mark_list_pattern(
        ...
        let pattern_strs: Vec<&str> = match original_value {
            ...
            _ => {
                return Err(RswappalyzerError::RuleParseError(format!(
                    "{}模式类型无效，仅支持字符串或数组",
                    pattern_type
                )));
            }
    ...
    fn clean_pattern_str_list(
        ...
        stats: &mut CleanStats,
        pattern_type: &str,
    ) -> RswResult<Vec<Pattern>> {
        let mut marked_patterns = Vec::new();
        ...
        for s in pattern_strs {
            ...
            if let Some(marked_pat) = self.process_single_pattern(s_trimmed, stats)? {
                marked_patterns.push(marked_pat);
                stats.update_valid_pattern_stats(pattern_type, 1);
            } else {
                stats.update_invalid_regex_stats(pattern_type, 1);
            }
    ...
    pub fn clean_and_mark_script_patterns(
        ...
        stats: &mut CleanStats,
    ) -> RswResult<(Option<PatternList>, Option<PatternList>)> {
    ...
    pub fn clean_and_mark_keyed_pattern(
        ...
        let mut valid_keyed_patterns = HashMap::new();
    ...
    pub fn process_single_pattern(
        ...
        match regex::Regex::new(cleaned_pattern_trimmed) {
            Ok(_) => {
                stats.regex_count += 1;
                // ✅ 核心修复：读取并使用is_fixed变量，根治赋值未读警告
                // 统计被修复过的正则总数，完美贴合业务设计初衷
                if is_fixed {
                    stats.fixed_regex_total_count += 1;
                }
                Ok(Some(Pattern {
                    pattern: cleaned_pattern_trimmed.to_string(),
                    match_type: MatchType::Regex,
                    version_template,
                }))
            }
            Err(e) => {
                warn!("正则模式 [{}] 修复后仍无效：{}", raw_pattern, e);
                Ok(None)
            }
        }
    }
